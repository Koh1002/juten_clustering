...
class ClusterLabeler:
    ...
    def _generate_with_llm(
        ...
        prompt = f"""以下は、企業の「取り組み宣言」テキストをクラスタリングした結果の一つです。
...
このクラスタを深く分析し、以下の形式で回答してください：

【ラベル】
このクラスタを端的に表す具体的なラベル（15〜25文字）。
「〜〜関連」「〜〜について」などの曖昧な表現は使わず、具体的な行動や目標を示すこと。
例：「省エネ設備への投資計画」「若手人材の育成プログラム」

【特徴説明】
このクラスタの本質を分析し、以下を含めて5〜8行で記述：

1. 共通する目的・意図：これらの宣言に共通する狙いや背景は何か
2. 具体的な取り組み内容：どのような施策・行動が含まれているか
3. 期待される効果・成果：これらの取り組みで何を達成しようとしているか
4. 推測される課題意識：なぜこのような宣言が集まったのか、背景にある課題は何か

単なるキーワードの羅列ではなく、分析的・洞察的な説明をしてください。
...
回答："""
    ...
    def _generate_fallback(
        ...
        if frequent_words:
            ...
            description = (
                f"このクラスタは「{words_desc}」などのキーワードを中心とした取り組み宣言で構成されています。"
                f"共通する目的として{frequent_words[0]}の推進・強化が挙げられ、"
                f"組織的な改善や成果向上を目指す施策が集まっていると考えられます。"
                f"（詳細な分析にはLLM APIの設定が必要です）"
            )
        else:
            label = f"クラスタ {cluster_id}"
            description = "このクラスタの特徴を分析中です。詳細な分析にはLLM APIの設定が必要です。"
